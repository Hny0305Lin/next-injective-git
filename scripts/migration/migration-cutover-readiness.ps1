...
& node (Join-Path $root "scripts/deploy/validate-suite-cutover.mjs") $evidence $ExpectedCommit | Out-Null
...
Write-Host "source gate: $(Join-Path $root 'scripts/ci/suite-readiness.sh') --required"
exit 0
