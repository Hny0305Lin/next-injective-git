...
CHAIN_ID="$(jq -er '.block.header.chain_id' <<<"$latest")"
LATEST_HEIGHT="$(jq -er '.block.header.height' <<<"$latest")"
HEIGHT="${HEIGHT:-$LATEST_HEIGHT}"
...
jq -nS \
  ...
  --arg lcd "$LCD" --arg contract "$CONTRACT" --arg chain_id "$CHAIN_ID" --arg height "$HEIGHT" \
  ...
  '{schema:$schema,source:{lcd:$lcd,contract:$contract,chain_id:$chain_id,height:$height,exported_at:$exported_at},contract_config:$config,upgrade_security:$upgrade,repositories:$repos,refs:$refs,collaborators:$collaborators,repo_extensions:$extensions,moderation_reports:$reports,usernames:$usernames,badges_by_recipient:$badges,releases:$releases}' \
  > "$tmp"
