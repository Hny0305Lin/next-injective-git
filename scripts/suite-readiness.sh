...
for file in migration-cutover-readiness.sh migration-cutover-readiness.ps1 validate-suite-cutover.mjs; do
  [[ -s "$ROOT/scripts/$file" ]] || { echo "FAIL: missing Suite cutover gate: scripts/$file" >&2; exit 1; }
done
