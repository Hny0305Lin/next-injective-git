...
if ! command -v node >/dev/null 2>&1; then
  ...
  node "$ROOT/scripts/ci/evm-suite-solc-check.mjs"
fi
for file in \
  foundry.toml \
...
if [[ ! -s "$ROOT/scripts/ci/evm-suite-solc-check.mjs" && ! -s "$ROOT/scripts/evm-suite-solc-check.mjs" ]]; then
  echo "FAIL: missing or empty EVM suite compiler gate: scripts/ci/evm-suite-solc-check.mjs (or shim)" >&2
  exit 1
fi
