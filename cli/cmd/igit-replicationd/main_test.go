...
func TestAuthorizeAcceptsServerCappedExpiration(t *testing.T) {
	s := &service{
		secret:   []byte("01234567890123456789012345678901"),
		maxBytes: 1024,
	}
	...
	ticket, err := s.sign(claims{
		Kind: "replication", Subject: "alice", CID: req.CID, Owner: req.Owner, Repo: req.Repo,
		Ref: req.Ref, PackSHA256: req.PackSHA256, Size: req.Size,
		ExpiresAt: time.Now().Add(15 * time.Minute).Unix(), JTI: "jti",
	})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := s.authorize("Bearer "+ticket, req); err != nil {
		t.Fatalf("server-capped ticket should remain valid: %v", err)
	}
}

func TestIdentityAcceptsSharedEd25519Token(t *testing.T) {
	publicKey, privateKey, err := ed25519.GenerateKey(nil)
	if err != nil {
		t.Fatal(err)
	}
	s := &service{identityKey: publicKey}
	header := base64.RawURLEncoding.EncodeToString([]byte(`{"alg":"EdDSA","typ":"JWT"}`))
	payload, err := json.Marshal(claims{Kind: "identity", Subject: "ip:test", ExpiresAt: time.Now().Add(time.Minute).Unix()})
	if err != nil {
		t.Fatal(err)
	}
	encodedPayload := base64.RawURLEncoding.EncodeToString(payload)
	message := header + "." + encodedPayload
	token := message + "." + base64.RawURLEncoding.EncodeToString(ed25519.Sign(privateKey, []byte(message)))
	identity, err := s.identity("Bearer " + token)
	if err != nil {
		t.Fatal(err)
	}
	if identity.Subject != "ip:test" {
